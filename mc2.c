...
// constraints.csv is the one that will be used in the algorithm and it is 72x72
...
// 3 - Xj / 3) > 2 4 = (Xi / 3 == Xj / 3 && Xi % 3 < Xj % 3) Do not care about
// what gets printed at output(just debugging)...check only .txt min-conflicts
// with random walk and random restarts Random restarts are about to be done
// here...at a new version upload
...
int RandomVariableConflict(int *Xvalue, int numberofvariables,
                           int numberofvalues);
int AlternativeAssignment(int *Xvalue, int numberofvariables, int variable,
                          int numberofvalues);
void minConflicts(int maxTries, int maxChanges, int *Xvalue,
                  int numberofvariables, int numberofvalues, FILE *outputFile,
                  int *moves, int *bestCollisions, double p);
...
int main() {
  ...
  for (int RestartsCounter = 0; RestartsCounter < PrecedureRestarts;
       RestartsCounter++) {
    ...
    minConflicts(maxTries, maxChanges, Xvalue, numberofvariables,
                 numberofvalues, outputFile, &moves, &bestCollisions, p);
    clock_t end = clock();
  ...
  double AverageBestCollisions =
      (double)totalBestCollisions / PrecedureRestarts;
...
int RandomVariableConflict(int *Xvalue, int numberofvariables,
  ...
  for (int i = 0; i < numberofvariables; i++) { // Check every variable
  ...
  return VariableWithConflicts[rand() % Counter]; // Return a random variable
                                                  // with conflict
}
...
int AlternativeAssignment(int *Xvalue, int numberofvariables, int variable,
  ...
  int TotalInitialConflicts =
      satisfies(Xvalue, numberofvariables,
                numberofvalues); // Number of total initial conflicts

  for (int NewValue = 0; NewValue < numberofvalues;
       NewValue++) { // Assign every value to every variable
  ...
  return BetterValue; // Return the alternative assignment of x which satisfies
...
void minConflicts(int maxTries, int maxChanges, int *Xvalue,
  ...
  for (int j = 0; j < maxChanges; j++) // maxChanges
  {
    ...
    if (randomNumber <=
        (int)(p * 100)) // if probability p verified (e.g i give 10%...if
                        // randomNumber <= 10 then p is verified)
    {
      // (x,a) := randomly chosen alternative assignment of x
      newAssignment = rand() % numberofvalues;
      // fprintf(outputFile, "(x,a) := randomly chosen alternative assignment of
      // x\n"); // debugging...will be removed
      fprintf(outputFile, "X%d new random value is: %d\n", x + 1,
              newAssignment);
    } else {
      // (x,a) := the alternative assignment of x which satisfies the maximum
      // number of constraints under the current assignment A
      newAssignment =
          AlternativeAssignment(Xvalue, numberofvariables, x, numberofvalues);
      // fprintf(outputFile, "(x,a) := the alternative assignment of x which
      // satisfies the maximum number of constraints under the current
      // assignment A\n"); // debugging...will be removed
